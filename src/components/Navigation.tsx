...
const Navigation = ({ name }: NavigationProps) => {
  ...
  return (
    <nav className="flex items-center justify-between md:pr-8 max-w-[1400px] w-[90vw] mx-auto flex-wrap md:flex-nowrap text-white">
      ...
      <ul
        className={`absolute top-16 left-0 md:static md:top-auto md:z-0 bg-black z-10 flex flex-col justify-center order-last md:order-none w-full md:w-auto md:flex-row gap-12 text-3xl md:gap-3 items-center transition-all ${
          toggleNav ? "h-[50vh]" : "h-0 hidden md:flex"
        }`}
      >
        <li className="hover:text-purple-500 hover:underline first-letter:text-purple-500" onClick={() => setToggleNav(false)}>
          <Link href="/">Home</Link>
        </li>
        <li className="hover:text-purple-500 hover:underline first-letter:text-purple-500" onClick={() => setToggleNav(false)}>
